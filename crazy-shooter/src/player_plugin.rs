...
impl Player {
    ...
    pub fn handle_state(mut player_query: Query<(&mut Player, &mut Transform)>, time: Res<Time>) {
        if let Ok((mut player, mut transform)) = player_query.get_single_mut() {
            ...
            if player.is_rotating() || player.is_reloading() {
                ...
                player.rotation_speed /= 1.1;
                transform.rotate_z(
                    rng.gen_range(if player.riffle_mode {
                        -5.0..2.
                    } else {
                        -3.0..1.5
